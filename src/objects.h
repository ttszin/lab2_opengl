...
class Objects {
    ...
    public:
        ...
        // Construtor (método especial chamado na criação do objeto)
        // Objects(std::string nome) {
        //     this->nome = nome;
        // }
        ...
        void desenhaJanela(float anguloAbertura) {
            ...
            for (float y_pos : {-altura / 2.0f, altura / 2.0f}) {
                ...
                glScalef(largura + espessura_moldura, espessura_moldura, espessura_moldura);
                desenhaCubo();
                glPopMatrix();
            }
            ...
            for (float x_pos : {-largura / 2.0f, largura / 2.0f}) {
                ...
                glScalef(espessura_moldura, altura - espessura_moldura, espessura_moldura);
                desenhaCubo();
                glPopMatrix();
            }
            glPopMatrix();
            ...
            glRotatef(this->anguloAbertura, 1.0f, 0.0f, 0.0f);
            ...
            glScalef(largura, altura, 0.02f); // Bem fino
            desenhaCubo();
        ...
        void desenhaMonitor() {
            ...
            glPushMatrix();
            glTranslatef(0.0f, 0.5f, 0.026f); // Levemente à frente da tela
            glScalef(0.65f, 0.45f, 0.01f);
            desenhaCubo();
            glPopMatrix();
        ...
        void desenhaMesaLaboratorio(float largura, float profundidade) {
            ...
            // Tampo
            glPushMatrix();
            glTranslatef(0.0f, altura - (espessura_tampo / 2.0f), 0.0f);
            glScalef(this->largura, espessura_tampo, this->profundidade);
            desenhaCubo();
            glPopMatrix();
        
            // Pés
            float pos_x = this->largura / 2.0f - espessura_pe / 2.0f;
            float pos_z = this->profundidade / 2.0f - espessura_pe / 2.0f;
            float alt_pe = altura - espessura_tampo;
        
            // Pé 1 (frente, direita)
            glPushMatrix();
            glTranslatef(pos_x, alt_pe / 2.0f, pos_z);
            glScalef(espessura_pe, alt_pe, espessura_pe);
            desenhaCubo();
            glPopMatrix();
        ...
        void desenhaPostoDeTrabalho() {
            ...
            // Monitor na mesa
            glPushMatrix();
            glTranslatef(0.0f, 0.7f, -0.1f); // 0.7 é a altura da mesa
            desenhaMonitor();
            glPopMatrix();
